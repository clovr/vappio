...
#This triggers a forced auto shutdown, just delayed
...
#Check for active jobs in SGE, unless forced
...
vlog "Scheduling forced shutdown of node $myhostname, nodetype:$nodetype"
if [ "$nodetype" == "exec" ]
then
...
if [ "$nodetype" == "master" ]
then
    #Check for recent tasks, unless forced
    if [ ! -f "$vappio_runtime/forceautoshutdown" ]
    then
	...
	then
	    verror "Master has been idle for $lasttaskmin minutes, forcing shutdown"
	    touch $vappio_runtime/forceautoshutdown
	else
	    vlog "Detected recent tasks on master in last $lasttaskmin minutes"
	    exit 0
	fi
    fi
    ...
    vp-terminate-cluster --cluster=local --auth-token=`generateAuthToken.py /mnt/keys/devel1.pem.pub`
else

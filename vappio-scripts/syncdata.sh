...
$SGE_ROOT/bin/$ARCH/qsub -o /mnt/scratch -e /mnt/scratch -S /bin/sh -b n -sync n -q $stagingq $seeding_script $node $stagingq 1>> $vappio_log 2>> $vappio_log
fi
done
...
$SGE_ROOT/bin/$ARCH/qsub -o /mnt/scratch -e /mnt/scratch -S /bin/sh -b n -sync n -q $stagingq,$stagingsubq $seeding_script $node $stagingsubq 1>> $vappio_log 2>> $vappio_log
fi
done

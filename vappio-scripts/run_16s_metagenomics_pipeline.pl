...
run_16s_metagenomics_pipeline.pl - Will start a cluster, upload necessary files, run a pipeline, monitor the pipeline.

=head1 SYNOPSIS

 USAGE: run_16s_metagenomics_pipeline.pl
       --16s_conf=/path/to/file.conf
       --help
...
=head1  DESCRIPTION
    ...
    The input section of this perldoc will describe the options for the 16s_conf file.  The clovr conf file should
...
sub start_pipeline {
    ...
    #build the command
    my $cmd = $runPipeline_exec." --name $cluster_tag --pipeline-name $pipeline_name --pipeline 16S_metagenomics ".
        "-- --INPUT_FILE_LIST $input_tag --GROUP_COUNT ". ($exec_nodes * 2)." --DB_PATH $reference_tag --conf $clovr_conf";
    $cmd .= " 2> $log";
...
sub init {
   ...
   if( $opts->{'16s_conf'} ) {
       $config = &parse_config( $opts->{'16s_conf'} );
   } else {
       die("Option 16s_conf is required");
   }

...
run_wgs_metagenomics_pipeline.pl - Will start a cluster, upload necessary files, run a pipeline, monitor the pipeline.

=head1 SYNOPSIS

 USAGE: run_wgs_metagenomics_pipeline.pl
       --wgs_conf=/path/to/file.conf
       --help

=head1 OPTIONS

B<--wgs_conf_file,-p>
    Used to specify all the options for a wgs metagenomics run
...
=head1  DESCRIPTION
    ...
    The input section of this perldoc will describe the options for the wgs_conf file.  The clovr conf file should
...
sub upload_files {
    ...
    my $input_sff_file = $cfg->val('input', 'input_fasta_file') or
            &_config_error( 'input', 'input_fasta_file');
...
sub start_pipeline {
    ...
    my $total_files = $cfg->val('input', 'total_files');
    my $cutoff = $cfg->val('input', 'cutoff');
    my $num_seqs = $cfg->val('input', 'num_seqs');
    ...
    #build the command
    my $cmd = $runPipeline_exec." --name $cluster_tag --pipeline-name $pipeline_name --pipeline total_metagenomics_stub ".
        "-- --INPUT_FILE_LIST $input_tag --TOTAL_FILES " . ($exec_nodes * 2) . " --CUTOFF $cutoff --GROUP_COUNT ".
        ($exec_nodes * 2)." --DB_PATH $reference_tag --conf $clovr_conf";
    $cmd .= " 2> $log";
...
sub init {
   ...
   if( $opts->{'wgs_conf'} ) {
       $config = &parse_config( $opts->{'wgs_conf'} );
   } else {
       die("Option wgs_conf is required");
   }

...
OPTIONS = [
    ('INPUT_TAG', '', '--INPUT_TAG', 'The input tag of sequences', compose(lambda x : '${dirs.tag_dir}/' + x, notNone)),
    ('REF_DB_TAG', '', '--REF_DB_TAG', 'The reference db for the blast run', compose(tagToRefDBPath, lambda x : '${dirs.tag_dir}/' + x, notNone)),
    ('PROGRAM', '', '--PROGRAM', 'The blast program to run (blastp, blastx, ..)', restrictValues(['blastn', 'blastp', 'blastx', 'tblastn', 'tblastx'])),

...
OPTIONS = [
    ('input.SHORT_PAIRED_LIST', '', '--SHORT_PAIRED_LIST', 'A comma separated list of input tags. Each tag should contain two files.', composeCLI( lambda x : ','.join(['${dirs.tag_dir}/' + y for y in x and re.split('[\s,]+', x) or []]), defaultIfNone('${input.SHORT_PAIRED_FILES}'))),
    ('input.LONG_PAIRED_LIST', '', '--LONG_PAIRED_LIST', 'A common separated list of input tags. Each tag should contain two files', composeCLI( lambda x : ','.join(['${dirs.tag_dir}/' + y for y in x and re.split('[\s,]+', x) or []]), defaultIfNone('${input.LONG_PAIRED_FILES}') ) ),
    ('input.SHORT_INPUT_LIST', '', '--SHORT_INPUT_LIST', 'Input tag for non paired end short read data.', composeCLI( lambda x : ','.join(['${dirs.tag_dir}/' + y for y in x.split()]), defaultIfNone('${input.SHORT_FILES}')) ),
    ('input.LONG_INPUT_LIST', '', '--LONG_INPUT_LIST', 'Input tag for non paired long read data', composeCLI( lambda x : ','.join(['${dirs.tag_dir}/' + y for y in x.split()]), defaultIfNone('${input.LONG_FILES}')) ),
